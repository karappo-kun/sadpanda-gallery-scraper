...
const preferSampled = PREFER_SAMPLED.trim().toLowerCase() === 'true';
...
async function downloadImages(galleryData) {
  ...
  while (pages.length) {
    const pagesOnQueue = pages.splice(0, 5);
  ...
  zip.file('gallery.json', JSON.stringify(galleryData.galleryInfo, null, 2));
